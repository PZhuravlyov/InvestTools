...
Database=AnalyticDev;Trusted_Connection=yes;"""
    ...
    def __set_archive_currencyratehistory_body(self):
        self.body = \
            {'dateFrom': self.fromDate, 'dateTo': self.toDate, 'baseCurrency': self.baseCurr, 'quotedCurrency': self.quotedCurr, 'step': 1440}

    # ---------------------------------------
    # ------- Bond controller actions -------
    # ---------------------------------------
    
    # Init all realized methods for Bond cotroller
    ...
    def __parse_bond_coupons_response(self):
        if self.roughData is None:
            return None
        dct = {'FinToolId': {}, 'CouponPeriod': {}, 'PeriodFrom': {}, 'PeriodTo': {}, 'PayPerBond': {}, 'CouponRate': {}}
        ...
        for datum in self.roughData:
            row = {'FinToolId': int(datum['id_fintool']), 'CouponPeriod': datum['id_coupon'],
                   'PeriodFrom': datum['begin_period'], 'PeriodTo': datum['end_period'], 'PayPerBond': datum['pay_per_bond'], 'CouponRate': datum['coupon_rate']}
            # parsed_data.append((datum['id_fintool'], datum['pay_date'], datum['period_from'], datum['period_to'], datum['pay_per_bond']))
    ...
    def __save_bond_coupons_from_parsed_data(self):
        ...
        insSQL = "insert into dbo.ACF_Coupons (FintoolId, CouponPeriod, StartDate, EndDate, [Value]) values(%s, %s, '%s', '%s', %f)"
        ...
        for index, row in self.savedData.iterrows():
            ...
            cursor.execute(insSQL % (row['FinToolId'], row['CouponPeriod'], row['PeriodFrom'][:10], row['PeriodTo'][:10], row['PayPerBond']))
            cursor.commit()
    ...
    def __set_info_calendar_body(self):
        today = date.today()
        self.body = {"eventTypes": ["DIV"],
          "fields": ["isiNcode","nickname","pay1Security", "fixDate","faceFTName","recomendFixDate","recomendPay1Security","rateDate"],
